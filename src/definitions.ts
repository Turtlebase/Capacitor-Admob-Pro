export interface AdMobAdvancedPlugin {
  initialize(options?: { testingDevices?: string[] }): Promise<void>;
  ...
  prepareRewarded(options: { adId: string }): Promise<any>;
  showRewarded(): Promise<any>;

  prepareRewardedInterstitial(options: { adId: string }): Promise<any>;
  showRewardedInterstitial(): Promise<any>;

  prepareAppOpen(options: { adId: string }): Promise<any>;
  showAppOpen(): Promise<void>;

  // 🔥 ADD THESE (fix your current crash)
  warmAll(): Promise<void>;
  isGoogleMobileAdsReady(): Promise<{ value: boolean }>;
}
